...
namespace WomenInWar.Helpers
{
    ...
    {
        private MonitorHelper(IntPtr handle)
        {
            Handle = handle;
        ...
        public static IEnumerable<MonitorHelper> All
        {
            get
            {
                var all = new List<MonitorHelper>();
                EnumDisplayMonitors(IntPtr.Zero, IntPtr.Zero, (m, h, rc, p) =>
                {
                    all.Add(new MonitorHelper(m));
                    return true;
                }, IntPtr.Zero);
                return all;
            }
        ...
        public static MonitorHelper FromWindow(IntPtr hwnd, MFW flags = MFW.MONITOR_DEFAULTTONULL)
        {
            var h = MonitorFromWindow(hwnd, flags);
            return h != IntPtr.Zero ? new MonitorHelper(h) : null;
        }
